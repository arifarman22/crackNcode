...
export const getDashboardStats = asyncHandler(async (_req: Request, res: Response) => {
  ...
  res.json({ users, products, orders, revenue: revenue._sum.total || 0, activeSubscriptions, recentOrders, recentUsers });
});
...
export const getChartData = asyncHandler(async (_req: Request, res: Response) => {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    revenueByMonth[key] = 0; ordersByMonth[key] = 0;
  }

  for (const o of orders) {
    const d = new Date(o.createdAt);
    const key = `${monthNames[d.getMonth()]} ${d.getFullYear()}`;
    if (key in revenueByMonth) { revenueByMonth[key] += o.total; ordersByMonth[key]++; }
  }
  ...
  const productsByCategory = await prisma.product.groupBy({
    by: ["category"], where: { active: true }, _count: true,
  });

  res.json({
    revenueChart, ordersChart,
    usersByRole: [{ name: "Admin", value: adminCount }, { name: "User", value: userCount }],
    ordersByStatus: [{ name: "Pending", value: pending }, { name: "Paid", value: paid }, { name: "Completed", value: completed }, { name: "Cancelled", value: cancelled }],
    productsByCategory: productsByCategory.map((p) => ({ name: p.category, value: p._count })),
  });
});
