...
import {
  ...
  ShoppingCart, Crown, ChevronLeft, ChevronRight, LogOut, Home,
} from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!hydrated) return;
    if (!user || !isAdmin(user.role)) router.push("/auth/login");
  ...
  return (
    <div className="flex min-h-screen -mt-16 pt-16">
      <aside
        className={`fixed top-16 left-0 h-[calc(100vh-4rem)] z-40 bg-surface-dark-card border-r border-white/10 transition-all duration-300 flex flex-col ${
          collapsed ? "w-16" : "w-60"
        }`}
      >
